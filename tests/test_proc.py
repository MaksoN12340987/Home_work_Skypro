...
@pytest.mark.parametrize("vareble, key, expected",[
    ([
...
def proc_filter_by_state(vareble, key, expected):
    assert filter_by_state(vareble, key) == expected


@pytest.mark.parametrize("vareble, reverse, expected",[
    (True),
    (False)
])
def proc_sort_by_date(list_data_bank_operation, reverse, expected):
